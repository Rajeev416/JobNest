...
const ViewApplications = () => {
  ...
  const fetchCompanyJobApplications = async (showLoading = true) => {
    try {
  ...
  const changeJobApplicationStatus = async (id, status) => {
    // Step 1: Immediately update the local state so the UI changes instantly
    ...
    // Step 2: Send the update to the backend in the background
    try {
  ...
  useEffect(() => {
    if (companyToken) {
      fetchCompanyJobApplications(true); // Initial load with spinner

      // Auto-refresh the board every 5 seconds for incoming applications
      const interval = setInterval(() => {
        fetchCompanyJobApplications(false); // Background refresh without spinner
      }, 5000);
    ...
    <div className="min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        <motion.div
          ...
          className="surface-panel rounded-2xl overflow-hidden mb-6"
        >
          {/* Filters and Search */}
          <div className="p-5 border-b border-gray-100 flex flex-wrap gap-4 items-center justify-between">
            <div className="relative flex-1 min-w-[250px]">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2.5 w-full rounded-lg border border-[#d7cec0] focus:ring-2 focus:ring-[#ff5c2a]/30 focus:border-[#ff5c2a] transition-colors"
                value={searchTerm}
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full">
              <thead className="bg-[#f2eadf] text-[#0b1220]">
                <tr>
                  ...
                  filteredApplicants.map((applicant, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-5">
                        ...
                        <span className="inline-flex items-center gap-2 bg-gray-50 text-gray-400 px-4 py-2 rounded-lg text-sm font-medium cursor-not-allowed">
                          <FileText size={16} />
                          No Resume
                        </span>
                        )}
